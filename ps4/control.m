...
transitions = zeros(NUM_STATES, NUM_STATES, 2);
val = rand(NUM_STATES,1)./10;
trans_prob = ones(NUM_STATES, NUM_STATES, 2)./NUM_STATES;
reward_counts = zeros(NUM_STATES, 2);
state_rewards = zeros(NUM_STATES,1);
...
% here, and say the overall algorithm has converged. 
consecs = 0;
while consecs<NO_LEARNING_THRESHOLD
% while (number of consecutive no learning trials < NO_LEARNING_THRESHOLD)
  ...
  % model.
  value_1 = trans_prob(state,:,1)*val;
  value_2 = trans_prob(state,:,2)*val;
  if (value_1>value_2)
    action=1;
  ...
  transitions(state,new_state,action) += 1;
  ...
  if (new_state==NUM_STATES)
    ...
    for a = 1:2
      for s = 1:NUM_STATES
        den = sum(transitions(s,:,a));
        if den > 0
          trans_prob(s,:,a) = transitions(s,:,a)/den;
        end
      end
    end
    ...
    for s = 1:NUM_STATES
      if (reward_counts(s,2)>0)
        state_rewards(s) = reward_counts(s,1)/reward_counts(s,2);
      end
    end
    ...
    iterations = 0;
    max_change = inf;
    new_val = zeros(NUM_STATES,1);
    while (max_change > TOLERANCE)
      for r = 1:NUM_STATES
        action_1 = trans_prob(r,:,1)*val;
        action_2 = trans_prob(r,:,2)*val;
        updated = max(action_1,action_2);
        new_val(r) = state_rewards(r)+GAMMA*updated;
      end
      max_change = max(abs(val-new_val));
      val = new_val;
      iterations += 1;
      disp(max_change);
      disp(iterations);
