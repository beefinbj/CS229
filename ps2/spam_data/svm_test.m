...
% YOUR CODE HERE
Xtest(Xtest>0) = 1.0;
Xtest_dots = Xtest*Xtrain';
Xtest_dot_self = sum(Xtest.^2,2);
tau = 8;

KMatTest = full(exp(-(repmat(Xtest_dot_self,1,numTrainDocs)+repmat(X_dot_self',numTestDocs,1)-2*Xtest_dots)/(2*tau^2)));

predictions = sign(KMatTest*average_alpha);
